...
%                       Default = []
%   thresholds          matrix of thresholds corresponding to test_nums for
%                       spike threshold level to use. Default from prefs.
%
%   OUTPUT ARGUMENTS
...
%Modified Amy Boyle 25 Jan 2011, to include different test spike thresholds
if ~exist('thresholds', 'var')
    thresholds = ones(size(test_nums))*prefs.spike_time_peak_threshold;
else
    if isempty(thresholds)
        thresholds = ones(size(test_nums))*prefs.spike_time_peak_threshold;
    else
        if length(thresholds) ~= length(test_nums)
            error('number of thresholds does not equal number of tests')
        end
    end
end
...
for test_num = test_nums
    ...
    peak_threshold = thresholds(test_threshold_idx);
    test_threshold_idx = test_threshold_idx + 1;
%     figure
%     hold on
...
%             plot(signal)
            ...
            smoothed_signal = Lowpass(abs(signal).^power_exponent,fs,filter_cutoff,4,1);  % Smooth the signal.^pw
%             figure
...
%                     warning(['Saturated MER signal detected for test ' int2str(test_num) ' trace ' int2str(trace_num) ' sweep ' int2str(sweep_num)]);
                    WriteStatus(['Saturated MER signal detected for test ' int2str(test_num) ' trace ' int2str(trace_num) ' sweep ' int2str(sweep_num)], 'orange');
                    signal_peaks = [];
                end
            end
