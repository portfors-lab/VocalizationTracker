...
for i = 1:N
    ...
    xw    = [xs(1+floor(alpha*wl))*ones(floor(alpha*wl),1); (xs(1+floor(alpha*wl)):xs(floor(end-alpha*wl)))'; xs(floor(end-alpha*wl))*ones(floor(alpha*wl),1)]; 
    y(i,1)  = mean(xs);
end
...
if exist('pf') & pf == 1,
    ...
    ylabel('Amplitude');
    grid on;
    title('Winsorized Trimmed Mean Filter');
    subplot(2,1,2)
    plot((0:N-1), (x-y), 'k');
    axis([0 N min(x)-4 max(x)+4]);
    xlabel('Samples');
