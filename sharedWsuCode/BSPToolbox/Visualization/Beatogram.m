function [B,t,d,pa] = Beatogram(x,fsa,bi,wla,doa,rga,nsa,pfa);
%Beatogram: Plots pulse morphology versus time as an image
%
%   [B,t,d,pa] = Beatogram(x,fs,bi,wl,do,rg,ns,pf);
%
%   x    Input signal    
...
rg = [0 1]; % Default range: -1 to 1 sec
rg = [-0.5 0.5];
...
ul = max( 1,ceil(rg(2)*fs));  % Upper limit, no less than 1 sample
