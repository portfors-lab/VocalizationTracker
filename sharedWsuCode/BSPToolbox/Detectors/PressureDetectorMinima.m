function [f] = PressureDetectorMinima(x,fsa,lfa,hfa,pfa)
%PressureDetectorMinima: Pressure detector algorithm (ICP, ABP)
%
%   f = PressureDetectorMinima(x,fs,lf,hf,pf);
%
%   x    Input signal       
...
%
%   Example: Find the percussion minima (start of beats) in the ICP signal;
%
%      load ICP; 
%      PressureDetectorMinima(icp, fs);
%
...
for k=1:wl:Lx                              
    if ((k+wl-1 < Lx-wl))
        xs       = x(k:k+wl-1);
        fss = pressuredetectminima(xs,fs,lf,hf,0);
        f = [f;fss+k-1];
    else
        xs       = x(k-wl:Lx);
        fss = pressuredetectminima(xs,fs,lf,hf,0);
