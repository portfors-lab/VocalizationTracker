%function [f] = PressureDetectv2(x,fsa,lfa,hfa,pfa)
%PressureDetectv2: Pressure detector algorithm (ICP, ABP)
%
%   f = PressureDetectv3(x,fs,lf,hf,pf);
%
%   x    Input signal       
...
%
%   Algorithm PRESSUREDETECTV3(x, [lf hf])
%   begin
...
xf3 = highpass(xf3,fs,(1/2)*lf,2,0);
f1  = [];
for k=1:wl:Lx-1,
    if ((k+wl-1 <= Lx-wl)),
        xs    = xf3(k:k+wl-1);
        thr   = prctile(xs,rthr);
        f1s   = DetectMaxima(xs,thr);
        f1    = [f1;f1s+k-1];
    else
        xs    = xf3(k:Lx);
        thr   = prctile(xs,rthr);
...
for k=1:Lmd,
    ...
    f1c = DetectMaxima(x2, prctile(x2,rthr));
    ftmp = [];
    for k2=1:length(f2c),
        i       = find(f1c > f2c(k2));
        [m, i2] = min(f1c(i)-f2c(k2));
        ftmp    = [ftmp; f1c(i(i2))];
    end;
    fc = [fc; ftmp+ni(k)];
end;
...
for k=1:Lmd90-1,
    x2  = xf3(ni(k):nf(k));
    f1c = DetectMaxima(x2, prctile(x2,rthr),0); 
    fc  = [fc; f1c+ni(k)];
end;
f = [f;fc];
f = unique(f);
