...
if exist('wda') & isstr(wda),
    if strcmpi(wda,'PPT') | strcmpi(wda,'PowerPoint'),                                 % PowerPoint
        wd = 8.5;
        ht = 5.6;
    elseif strcmpi(wda,'Pukoa'),               
        wd = 10;
        ht = 5.0;       
...
previousUnits = get(fh, 'units');
set(fh, 'units', get(0, 'units'));
...
for c1 = 1:length(fh)
    if iscell(previousUnits), set(fh(c1),'units',previousUnits{c1});
    else                      set(fh(c1),'units',previousUnits);      end
end
%drawnow;
previousUnits = get(gcf, 'units');
set(gcf, 'units', get(0, 'units'));
set(gcf,'Position',sp);
set(gcf, 'units', previousUnits);
